"""
spark-submit --conf  spark.dynamicAllocation.enabled=true --conf spark.shuffle.service.enabled=false --conf spark.dynamicAllocation.shuffleTracking.enabled=true data_preprossing/Data_spliting_small.py ml-latest-small/ratings.csv
spark-submit data_preprossing/Data_spliting_small.py ml-latest-small/ratings.csv
"""
...
def main(spark, file):
    data_small = spark.read.csv(file, header=True, schema='userId INT, movieId INT, rating FLOAT, timestamp INT')
    ...
    val_filtered.write.parquet('data/small_val.parquet', mode="overwrite")
    test_filtered.write.parquet('data/small_test.parquet', mode="overwrite")
